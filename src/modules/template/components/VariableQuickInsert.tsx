...
export function VariableQuickInsert({ onInsert, customVariables }: VariableQuickInsertProps) {
  ...
  const defaultVariables = [
    { label: t("template.variables.name") || "Nama Pelanggan", key: "nama" },
    { label: t("template.variables.phone") || "Nomor WhatsApp", key: "nomor" },
    { label: t("template.variables.email") || "Email", key: "email" },
    { label: t("template.variables.date") || "Tanggal Transaksi", key: "tanggal" },
    { label: t("template.variables.invoice") || "Nomor Faktur / Invoice", key: "invoice" },
    { label: t("template.variables.link") || "Link Tautan", key: "link" },
  ];

  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center justify-between text-xs text-foreground-muted">
        <span className="font-medium">{t("template.variables.quickTitle") || "Variabel Cepat (Klik untuk menyisipkan):"}</span>
        <span className="text-[11px] opacity-75">{t("template.variables.format") || "Format: {{variabel}}"}</span>
      </div>
      <div className="flex flex-wrap gap-1.5 py-0.5">
        {defaultVariables.map((v) => (
          <button
            ...
            title={`${t("template.variables.insertTooltip") || "Sisipkan"} {{${v.key}}} (${v.label})`}
          >
        ...
        {customVariables?.map((v) => (
          <button
            ...
            title={`${t("template.variables.insertTooltip") || "Sisipkan"} {{${v}}}`}
          >
